...
class AccountMove(models.Model):
    ...
    pending_date = fields.Date(
        readonly=True,
        help="Date (not included) until the widget to validate purchase "
        "invoice/refund will ignore this document from the queue",
    )
    ...
    def _compute_validation_user_id(self):
        for rec in self:
            ...
            if rec.move_type in rec.get_concerned_types() and not rec.validation_user_id:
                rec.validation_user_id = rec.company_id.validation_user_id
            else:
                #no change
                rec.validation_user_id = rec.validation_user_id


    def _compute_can_edit_validation_user(self):
        for rec in self:
            rec.can_edit_validation_user = rec.env.user.has_group("account.group_account_manager")

    @api.depends("validation_user_id")
    def _compute_validation_state(self) -> None:
        for rec in self:
            if rec.validation_user_id:
                rec.validation_state = "wait_approval"
