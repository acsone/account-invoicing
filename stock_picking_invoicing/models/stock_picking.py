...
class StockPicking(models.Model):
    ...
    def set_invoiced(self):
        ...
        for picking in self:
            ...
            moves_to_update |= picking.move_lines
        # To avoid too many re-computation, execute update on many records
        pickings_to_update._set_as_invoiced()
        moves_to_update._set_as_invoiced()
        return True
            ...
            for move in picking.move_lines:
                if move.invoice_state == '2binvoiced':
                    if (move.state != 'cancel') and not move.scrapped:
                        todo.setdefault(key, []).append(move)
        invoices = self.env['account.invoice'].browse()
